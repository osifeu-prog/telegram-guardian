...
os.environ['TON_TREASURY_ADDRESS'] = 'UQCr743gEr_nqV_0SBkSp3CtYS_15R3LDLBvLmKeEv7XdGvp'
...
from web_portal.app.tg_bot import (
...
def mock_redis(monkeypatch):
    ...
    # you can add mocking logic if needed
...
@pytest.fixture
def mock_db():
    with patch('web_portal.app.tg_bot.SessionLocal') as mock_session:
        mock_db = MagicMock()
...
async def call_with_db(handler, update, context, db_mock):
    with patch('web_portal.app.tg_bot.SessionLocal', return_value=db_mock):
...
async def test_cmd_manh(mock_db):
    ...
    update.message.reply_text = AsyncMock()
    context = MagicMock()
    with patch('web_portal.app.tg_bot.get_balance', return_value=100):
...
async def test_cmd_leaderboard(mock_db):
    ...
    with patch('web_portal.app.tg_bot.get_leaderboard', return_value=mock_leaderboard):
...
async def test_cmd_buy(mock_db):
    ...
    update.message.reply_text = AsyncMock()
    context = MagicMock()
    with patch('web_portal.app.tg_bot.create_invoice') as mock_create:
        mock_inv = MagicMock()
...
async def test_cmd_invoices(mock_db):
    ...
    with patch('web_portal.app.tg_bot.list_invoices', return_value=[mock_invoice]):
...
async def test_cmd_poll_confirm(mock_db):
    ...
    update.message.reply_text = AsyncMock()
    context = MagicMock()
    with patch('web_portal.app.tg_bot.poll_and_confirm_invoices', return_value={'confirmed': 1}):
...
async def test_cmd_withdraw(mock_db):
    ...
    update.message.reply_text = AsyncMock()
    context = MagicMock()
    with patch('web_portal.app.tg_bot.create_withdrawal') as mock_create:
...
async def test_cmd_withdrawals(mock_db):
    ...
    with patch('web_portal.app.tg_bot.get_user_withdrawals', return_value=[mock_w]):
...
async def test_cmd_p2p_buy(mock_db):
    ...
    context = MagicMock()
    context.args = ['10', '5']
    with patch('web_portal.app.tg_bot.get_redis', return_value=MockRedis()):
...
async def test_cmd_referral(mock_db):
    ...
    with patch('web_portal.app.tg_bot.set_referral_code', return_value='R12345'):
